...
const tubo = document.querySelector('.tubo'); // Elemento do obstáculo "Tubo"
...
let passedTubo = false; // Controle para verificar se o personagem passou pelo obstáculo
...
const checkCollision = setInterval(() => {
    ...
    const tuboLeft = parseInt(window.getComputedStyle(tubo).getPropertyValue('left')); // Posição do obstáculo
    const tuboHeight = parseInt(window.getComputedStyle(tubo).getPropertyValue('height')); // Altura do obstáculo
    ...
    if (tuboLeft > collisionAreaStart && tuboLeft < collisionAreaEnd && rodrigoBottom <= tuboHeight) {
...
function gameOver() {
    ...
    tubo.style.animation = 'none'; // Para a animação do obstáculo
...
function resetGame() {
    ...
    passedTubo = false; // Reseta a variável de controle de passagem pelo obstáculo
    ...
    tubo.style.animation = 'none'; // Remove a animação do obstáculo
    tubo.style.left = '100%'; // Reposiciona o obstáculo fora da tela
